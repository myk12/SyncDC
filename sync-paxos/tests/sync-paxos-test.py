...
async def run_simulation_once() -> float:
    ...
    while not node_lists[0].alive:
        ...
        sim_time += config.NUM_NODES * config.MEAN_DELAY_US / 1e6
    ...
    # Message Rounds
    sim_time += 3 * config.MEAN_DELAY_US / 1e6
    return sim_time
...
def plot_cdf():
    ...
    plt.grid(True)
    plt.legend()
    plt.savefig('sync_consensus_cdf.png')
    plt.close()
...
async def main():
    ...
    with open('sync-paxos-test.csv', 'w') as f:
