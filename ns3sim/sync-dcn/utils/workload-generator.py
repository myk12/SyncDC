...
def parse_args():
    ...
    parser.add_argument("--burst_interval", type=float, default=0.5, 
                        help="Interval between bursts in seconds (default: 60.0)")
    parser.add_argument("--burst_duration", type=float, default=0.1, 
...
def main():
    ...
    data = {
        ...
        'request_servers': [random.choice(request_nodes_options) for _ in range(num_jobs)],
        'create_time': create_times,
        'message_size': [random.choice(data_size_options) for _ in range(num_jobs)]
    }
